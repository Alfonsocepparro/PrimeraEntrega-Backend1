...
class ProductsManagerFs {
    ...
    getProducts = async () => { //traer todos
        const products = await this.readProducts()
        return products
    }

    getProduct = async (objectparams) => { //traer uno
    ...
    createProduct = async newProduct => { //crear uno
        try {
            const products = await this.readProducts()
    ...
    updateProduct = async (objparams, objbody) => { //modificar un prod
        const { pid } = objparams
    ...
    deleteProduct = async (objparams) => { //borrar 
        const { pid } = objparams
        const allProducts = await this.readProducts()
        const productsNoEncontrados = allProducts.filter(
            (elemento) => elemento.id !== parseInt(pid)
        )
        await fs.promises.writeFile(this.path, JSON.stringify(productsNoEncontrados, null, '\t'))
    }
